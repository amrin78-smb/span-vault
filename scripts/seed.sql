...
-- Run after schema.sql to populate with example data for testing
...
INSERT INTO icmp_targets (device_id, ip_address, label, priority) VALUES
  ...
  (4, '10.2.1.1',  'Samut Sakhon FW',    'normal'),
  (5, '10.2.1.2',  'Samut Sakhon RT',    'normal'),
  (6, '10.3.1.1',  'Songkhla FW',        'normal'),
  (7, '10.4.1.1',  'Singapore Router',   'critical'),
...
INSERT INTO topology_nodes (device_id, label, node_type, x, y, site_id) VALUES
  ...
  (7, 'SG Router',         'router',   400, 700, 4)
ON CONFLICT DO NOTHING;

-- Topology Links
INSERT INTO topology_links (source_node_id, target_node_id, label, link_speed_bps) VALUES
  (1, 2, 'HQ FW to RT',   1000000000),
  (2, 3, 'HQ RT to SW',   10000000000),
  (2, 5, 'HQ to SS MPLS', 100000000),
  (2, 6, 'HQ to SK MPLS', 100000000),
  (2, 7, 'HQ to SG MPLS', 1000000000)
ON CONFLICT DO NOTHING;
